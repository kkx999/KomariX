...
# Global variables
INSTALL_DIR="/opt/komarix"
DATA_DIR="/opt/komarix"
SERVICE_NAME="komarix"
BINARY_PATH="$INSTALL_DIR/komarix"
...
show_banner() {
    ...
    clear
    echo "=============================================================="
    echo "            KomariX Monitoring System Installer"
    echo "       https://github.com/kkx999/KomariX"
    echo "=============================================================="
    echo
}
...
# Check if KomariX is already installed
is_installed() {
...
install_binary() {
    ...
    while true; do
        local input_port
        input_port=$(ui_input "监听端口" "请输入 KomariX 的监听端口 (1-65535)：" "$DEFAULT_PORT")
        # 取消输入
        if [ $? -ne 0 ]; then
            log_info "安装已取消"
            return
...
[Unit]
Description=KomariX Monitor Service
...
cleanup_backups() {
    if ! ui_yesno "确认清理备份" "将删除 KomariX 的二进制升级备份和数据升级压缩包。\n\n您确定要继续吗？"; then
        log_info "备份清理已取消"
        return 0
    fi
...
upgrade_komarix() {
    ...
    if ! is_installed; then
        ui_msgbox "错误" "KomariX 未安装。请先安装它。"
        return 1
    fi
...
uninstall_komarix() {
    ...
    if ! is_installed; then
        ui_msgbox "提示" "KomariX 未安装。"
        return 0
    fi

    if ! ui_yesno "确认卸载" "这将删除 KomariX 二进制文件和服务。\n\n您确定要继续吗？"; then
        log_info "卸载已取消"
        return 0
    fi
    ...
    log_success "KomariX 二进制文件已删除"

    ui_msgbox "卸载完成" "KomariX 卸载完成。\n\n数据文件保留在 $DATA_DIR"
}

# Show service status
show_status() {
    if ! is_installed; then
        ui_msgbox "错误" "KomariX 未安装。"
        return
    fi
    if ! check_systemd; then
...
show_logs() {
    ...
    log_step "查看 KomariX 服务日志 (按 Ctrl+C 退出)..."
...
main_menu() {
    while true; do
        ...
        local choice
        choice=$(ui_menu "KomariX 监控系统安装器" "请选择操作：" \
            "1" "安装 KomariX" \
            "2" "升级 KomariX" \
            "3" "卸载 KomariX" \
            "4" "查看状态" \
