...
func TestBuiltinStyleStreamingWithResponseHooks(t *testing.T) {
	...
	hookZip := writePluginZip(t, map[string]string{
		"komarix-plugin.json": `{"name":"Hook","short":"hook","version":"1.0.0","permissions":{"timeout":5,"allowHooks":true}}`,
...
func TestRouteStreamingMJPEG(t *testing.T) {
	...
	zipPath := writePluginZip(t, map[string]string{
		"komarix-plugin.json": `{"name":"Mjpeg","short":"mjpeg","version":"1.0.0","permissions":{"node":true,"timeout":5,"allowRoutes":true}}`,
		"script.js":          script,
	})
