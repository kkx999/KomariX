...
`fake_agent.py` creates a test-only KomariX v2 client that reports generated system metrics. It only advertises and processes `agent.ping`; it does not connect to, advertise, or process terminal or command-execution requests.

Create and persist a new client locally:

```powershell
python .\fake_agent.py --server https://komarix.example.com --adkey YOUR_AUTO_DISCOVERY_KEY -new
```

The generated client token is saved in `fake-agent-client.json` alongside the script. Subsequent runs reuse it:

```powershell
python .\fake_agent.py --server https://komarix.example.com --adkey unused
```
