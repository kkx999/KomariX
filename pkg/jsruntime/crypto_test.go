...
func TestCryptoCiphers(t *testing.T) {
	runtime := newCryptoTestRuntime(t, `
		...
		async function verify() {
			...
			const cbcParts = [cbc.update("KomariX is a monitoring platform with a lot of data")];
			...
			if (joinBuffers(decbcParts).toString() !== "KomariX is a monitoring platform with a lot of data") return "cbc";
			// CBC with no padding.
